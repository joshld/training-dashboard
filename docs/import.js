...
async function parseFit(file){
  let fitSdk;
  try{
    fitSdk=await import('https://esm.sh/@garmin/fitsdk@21.208.0');
  }catch(error){
    throw new Error('The FIT decoder could not be loaded. Check your connection or try a TCX export.');
  }
...
function childText(node,name){
  const elements=[...node.getElementsByTagNameNS('*',name)];
  return elements.length?elements[0].textContent?.trim()||null:null;
}
function parseXml(file,xml,type){
  const doc=new DOMParser().parseFromString(xml,'application/xml');
  if(doc.querySelector('parsererror'))throw new Error(`Could not parse ${type} XML.`);
  const isTcx=type==='TCX';
  const points=isTcx?[...doc.getElementsByTagNameNS('*','Trackpoint')]:[...doc.getElementsByTagNameNS('*','trkpt')];
  if(!points.length)throw new Error(`No track points were found in this ${type} file.`);
  const times=points.map(p=>Date.parse(text(p,isTcx?'Time':'time'))).filter(Number.isFinite);
  const hrs=points.map(p=>{
    if(isTcx){const hrNode=p.getElementsByTagNameNS('*','HeartRateBpm')[0];return num(text(hrNode,'Value'));}
    return num(text(p,'hr'));
  }).filter(v=>v!=null);
  const cads=points.map(p=>num(text(p,isTcx?'Cadence':'cad'))).filter(v=>v!=null);
  let distanceM=isTcx?num(text(doc,'DistanceMeters')):null;
  if(distanceM==null&&!isTcx)distanceM=calculateGpxDistance(points);
  ...
  const laps=isTcx?[...doc.getElementsByTagNameNS('*','Lap')].map((lap,i)=>{const d=num(childText(lap,'DistanceMeters'));const t=num(childText(lap,'TotalTimeSeconds'));const hrNode=lap.getElementsByTagNameNS('*','AverageHeartRateBpm')[0];return{index:i+1,distanceKm:d!=null?d/1000:null,durationSeconds:t,pace:fmtPace(t,d!=null?d/1000:null),averageHeartRate:num(text(hrNode,'Value'))};}):[];
...
function render(summary){currentSummary=summary;document.getElementById('activityTitle').textContent=`${summary.activityType||'Activity'}`;document.getElementById('activityFormat').textContent=summary.sourceFormat;
  ...
  status.innerHTML=`<p class="status-ok">Parsed ${esc(summary.fileName)} successfully.</p>`;preview.hidden=false;
}
